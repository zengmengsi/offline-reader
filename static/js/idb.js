(function() {
  ...
  IDBHelperProto.len = function(storeName) {
      return this.transaction(storeName, function(tx) {
          return tx.objectStore(storeName).count();
      });
      // var transaction=this.transaction(storeName);
  ...
  IDBHelperProto.del = function(storeName, key) {
      return this.transaction(storeName, function(tx) {
          tx.objectStore(storeName).delete( key);
      }, {
          mode: 'readwrite'
      });
      // var transaction=this.transaction(storeName,'readwrite');
      // var store=transaction.objectStore(storeName);
      // store.delete(key);
      // return true;
  };
